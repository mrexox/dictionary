...
main(int argc, char** argv) {
  printf("Hello!\n"
	 ...
	 "Usage: \\a 'any word'|\"any word\"|word meaning till new line -- adding new word\n"
	 "       \\l -- list all added words\n"
	 "       \\w file.txt -- write dictionary to file\n"
	 "       \\f 'any word'|\"any word\"|word -- find meaning of the word\n"
	 "       \\d 'any word'|\"any word\"|word -- delete the word from dictionary\n"
	 "       \\q -- exit the program\n"
	 "Run: `./dict my_dictionary_file.txt` to load the dictionary file\n"
	 "------------------------------------------\n");
  ...
  if (argc == 2) {
    ...
    if (loadfile != NULL) {
      int err = load_dictionary(loadfile);
      switch (err) {
      case UNEXPECTED_SYNTAX:
	printf("Unexpected syntax was cought. Check the file %s\n", argv[1]);
	break;
      case COLLISION:
	printf("Collision while loading.\nLoading stopped.\n");
	break;
      }
      fclose(loadfile);
    } else {
